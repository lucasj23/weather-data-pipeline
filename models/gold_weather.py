...
def build_gold(run_date: str | None = None) -> str:
    ...
    # Basic validations
    needed = {"run_date","city_code","date","temp_max","temp_min","temp_avg","temp_range","precip_mm"}
    ...
    # --------------------
    daily_kpi = (df.groupby(["city_code","date"], dropna=False)
                   .agg(
    ...
    daily_kpi["avg_max_7d"] = (daily_kpi
                               .groupby("city_code")["temp_max"]
                               .transform(lambda s: s.rolling(7, min_periods=1).mean()))
    daily_kpi["avg_min_7d"] = (daily_kpi
                               .groupby("city_code")["temp_min"]
                               .transform(lambda s: s.rolling(7, min_periods=1).mean()))
    daily_kpi["avg_max_14d"] = (daily_kpi
                                .groupby("city_code")["temp_max"]
                                .transform(lambda s: s.rolling(14, min_periods=1).mean()))
    daily_kpi["avg_min_14d"] = (daily_kpi
                                .groupby("city_code")["temp_min"]
                                .transform(lambda s: s.rolling(14, min_periods=1).mean()))
    ...
    prev = prev.rename(columns={
        "temp_min": "temp_min_ly",
        "temp_max": "temp_max_ly"
    })
    ...
    if "temp_min_ly" in daily_kpi.columns:
        daily_kpi["temp_min_yoy_pct"] = _pct(daily_kpi["temp_min"], daily_kpi["temp_min_ly"])
    if "temp_max_ly" in daily_kpi.columns:
        daily_kpi["temp_max_yoy_pct"] = _pct(daily_kpi["temp_max"], daily_kpi["temp_max_ly"])

    # --------------------
    # MONTHLY KPIs (city-month)
    # --------------------
    monthly_kpi = (df.groupby(["city_code","yyyymm"], dropna=False)
                     .agg(
                         days=("date","nunique"),
    ...
    monthly_kpi["avg_temp_roll3"] = (monthly_kpi
                                     .groupby("city_code")["avg_temp"]
                                     .transform(lambda s: s.rolling(3, min_periods=1).mean()))

    
    # --- Round numeric metrics to 1 decimal ---
    for col in [
        "temp_max","temp_min","temp_avg","temp_range","precip_mm",
        "avg_max_7d","avg_min_7d","avg_max_14d","avg_min_14d",
        "temp_min_ly","temp_max_ly","temp_min_yoy_pct","temp_max_yoy_pct"
    ]:
        if col in daily_kpi.columns:
            daily_kpi[col] = daily_kpi[col].round(1)

    for col in [
        "avg_temp","max_temp","min_temp","avg_range","total_precip_mm",
        "avg_temp_roll3"
    ]:
    ...
    daily_kpi[["city_code","date","temp_max","temp_min","temp_avg","temp_range","precip_mm"]].to_parquet(
        daily_parquet, index=False
    )
    monthly_kpi.to_parquet(monthly_parquet, index=False)

    # CSV samples
    daily_kpi.head(200).to_csv(os.path.join(out_dir, "weather_daily_enriched_sample.csv"), index=False)
    monthly_kpi.head(200).to_csv(os.path.join(out_dir, "weather_monthly_kpis_sample.csv"), index=False)
